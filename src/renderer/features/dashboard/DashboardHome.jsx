...
export default function DashboardHome() {
    ...
    const [activeTab, setActiveTab] = useState('general'); // 'general' or 'inventory'
    ...
    return (
        <div className="space-y-6 h-full flex flex-col overflow-y-auto custom-scrollbar p-1">
                ...
                <div className="flex bg-slate-900/80 p-1 rounded-xl border border-white/5">
                    <button
                        ...
                        className={cn(
                            ...
                            activeTab === 'general' ? "bg-blue-600 text-white shadow-lg" : "text-slate-500 hover:text-slate-300"
                        )}
                    >
                        GENERAL
                    </button>
                    <button
                        ...
                        className={cn(
                            "px-4 py-1.5 text-xs font-bold rounded-lg transition-all",
                            activeTab === 'inventory' ? "bg-emerald-600 text-white shadow-lg" : "text-slate-500 hover:text-slate-300"
                        )}
                    >
                        ALMACÉN
                    </button>
            ...
            {activeTab === 'general' ? (
                <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <KPICard
                            ...
                            value={new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(data.totalRevenueAllTime || totalRevenue)}
                            icon={CreditCard}
                            color="text-emerald-400"
                            trend={revGrowth > 0 ? `+${revGrowth.toFixed(1)}%` : `${revGrowth.toFixed(1)}%`}
                            trendUp={revGrowth >= 0}
                            sub={`Año ${selectedYear}: ${new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR' }).format(totalRevenue)}`}
                        />
                        <KPICard
                ...
                <InventoryDashboard data={inventoryData} />
            )}
        </div>
    );
}
